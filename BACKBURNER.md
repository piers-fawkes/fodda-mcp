...
## 📐 Tool `outputSchema`s + `structuredContent` (directory quality nudge)
**Status:** Backburner — added 2026-09-03
**What:** The ChatGPT Apps Directory editor recommends (does not require) an `outputSchema` per tool so models parse results better. SDK `@modelcontextprotocol/sdk@1.27.1` supports it (`registerTool({ outputSchema })`).
**The gotcha (why this is bigger than it looks):** per the SDK, a tool that declares `outputSchema` MUST return `structuredContent` matching it — the SDK returns `structuredContent` when an outputSchema is defined and `content` otherwise, and validating clients (incl. ChatGPT) will reject a text-only return against a declared schema. Today **0 of 24 handlers return `structuredContent`** — every tool returns `content: [{ type: 'text', text: JSON.stringify(...) }]`. So this is NOT a "just add a schema" change; each tool needs BOTH a schema AND its handler changed to emit `structuredContent` (keep the text block for back-compat).
**Scope:** 24 tools with heterogeneous, deeply-nested shapes (e.g. `search_graph` → trends[] with evidence[], coverage annotations; `get_company_earnings` → SWOT/sentiment/activity records; `brand_tracker` → footprint + competitive + supplemental blocks). Do NOT big-bang all 24. Sequence: (1) the simple, high-value reads first — `get_my_account`, `list_graphs`, `list_analysts`, `get_capabilities`, `check_research_status`, `check_supplemental_status`; (2) the search family — `search_graph`, `search_statistics`, `search_insights`, `get_evidence`, `get_node`, `get_neighbors`, `get_label_values`; (3) earnings + brand + research — `get_company_earnings`, `get_earnings_*`, `get_validated_trends`, `brand_tracker`, `deep_research_topic`; (4) `consult_*`, `read_url`, `get_supplemental_context`, `generate_visual`.
**Per-tool recipe:** define a zod/JSON output schema from the ACTUAL live return (call the tool, capture the shape — don't guess); add `outputSchema` to the registration; change the return to `{ content: [text...], structuredContent: <object> }`; verify with MCP Inspector that structuredContent validates and no client errors. Add a build-time check that any tool with an outputSchema also returns structuredContent.
**Definition of Done:** each converted tool declares an accurate `outputSchema`, returns matching `structuredContent`, still returns the text block, passes MCP Inspector + a live `/chatgpt` session with no validation errors; CHANGELOG per batch.
**Do NOT:** declare an `outputSchema` without also emitting `structuredContent` (breaks validating clients — worse than no schema); guess shapes from descriptions instead of live output; ship all 24 in one commit.
**Not a submission blocker** — outputSchema is a recommendation, not a requirement; the listing can go live without it.
**Agent:** MCP agent
