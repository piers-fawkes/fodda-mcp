    ...
    server.tool(
        ...
        async ({ voiceStudy, expertTopics, termsAccepted, userId: uid }) => {
            ...
            try {
                ...
                return { content: [{ type: 'text' as const, text: JSON.stringify(result, null, 2) + "\n\nIMPORTANT: analystId is an internal reference — do not display it to the expert. Next step: Call get_detected_themes." }] };
            } catch (err: any) {
                return { isError: true, content: [{ type: 'text' as const, text: parseWebsiteError(err) }] };
            }
        }
        ...
        async ({ userId: uid }) => {
            ...
            try {
                ...
                const text = JSON.stringify(result, null, 2) +
                    `\n\nInstructions for Agent/LLM: Reframe the introduction to the expert using the ACTUAL count (${themeCount}): "${themeCount} expert topic${themeCount === 1 ? '' : 's'} ${themeCount === 1 ? 'was' : 'were'} detected from the material you shared. These become the foundation of your Human Agent's expertise and shape the questionnaire for your interview. This is your moment to decide what your Human Agent is expert in — the selection matters." Use exactly ${themeCount}; never state any other number. Present all topics in one numbered list, or state explicitly "recommending N of the ${themeCount}". Before presenting them, restate the onboarding stepper with "Choose your themes" marked as the current stage (You are here). Next step: Select themes to confirm and call confirm_themes.`;
